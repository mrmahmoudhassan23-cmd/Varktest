...
export default function StudentTest(){
 ...
 const [loading,setLoading]=useState(true); const [name,setName]=useState(''); const [listNumber,setListNumber]=useState(''); const [step,setStep]=useState(0); const [answers,setAnswers]=useState<number[]>([]); const [scores,setScores]=useState<Scores>(initial); const [saving,setSaving]=useState(false); const [done,setDone]=useState(false); const [error,setError]=useState('');
 ...
 if(step===0)return <main className="shell"><section className="card"><div className="brand"><span className="badge">EBIS • YEAR 7 • {session?.class_name}</span><h1>How Do You Learn?</h1><p className="muted">Answer each question with the option that sounds most like you. There are no right or wrong answers.</p></div><div className="field"><label>Full name</label><input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder="Enter your full name"/></div><div className="field"><label>List number</label><input inputMode="numeric" value={listNumber} onChange={e=>setListNumber(e.target.value.replace(/\D/g,''))} placeholder="e.g. 12"/></div><button className="primary" disabled={!name.trim()||!listNumber.trim()} onClick={()=>setStep(1)}>Start the challenge →</button></section></main>;
